...
import { PlayerSelect } from 'components/common/form-inputs';
import { DEFAULT_OVERLAY, Division } from 'components/constants';
...
export const CalcuttaPartner = (props: CalcuttaPartnerProps) => {
  ...
  const { isPending, isSuccess, isError, error, data } = useCalcuttaPartner(player.id);

  const createCalcuttaTeamMutation = useMutation({
    ...
    onSettled: () =>
      queryClient.invalidateQueries({ queryKey: ['calcutta/teams/partner', player.id] }),
  });

  const deleteCalcuttaTeamMutation = useMutation({
  ...
  const addPartner = () => {
    ...
    createCalcuttaTeamMutation.mutate(
  ...
  const removePartner = () => {
    ...
    deleteCalcuttaTeamMutation.mutate(data!.id, {
      onSettled: (response) => {
        if (response) {
...
export const CalcuttaPartnerSelect = (props: CalcuttaPartnerSelectProps) => {
  ...
  const { status, error, data } = useAvailablePartners(division);
